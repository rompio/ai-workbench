...
class PInfoTestCase(TestCase):
    def setUp(self):
        ...
        self.pinfo = PInfo.objects.create(
            first_name='John', last_name='Doe', email='john.doe@example.com', user=user
        )
...
class ApplicationTestCase(TestCase):
    def setUp(self):
        user = CustomUser.objects.create_user(username='testuser', password='password')
        offer = Offer.objects.create(position='Software Engineer', company='TechCorp', user=user)
    ...
    def test_application_str(self):
        self.assertEqual(str(self.application), 'Application by testuser for Software Engineer')

class ChatLogTestCase(TestCase):
    def setUp(self):
        user = CustomUser.objects.create_user(username='testuser', password='password')
        self.chat_log = ChatLog.objects.create(
            user_input='What is the status of my application?', assistant_response='Your application is under review.', user=user
        )

    def test_chat_log_creation(self):
        self.assertEqual(self.chat_log.user_input, 'What is the status of my application?')
        self.assertEqual(self.chat_log.assistant_response, 'Your application is under review.')
